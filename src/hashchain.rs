...
use plonky2::plonk::proof::{CompressedProofWithPublicInputs, ProofWithPublicInputs, Proof};
...
// Recursively validated proof of hashchain.
...
pub fn ground_proof<F: RichField + Extendable<D>, C: GenericConfig<D, F=F>, const D: usize, const B: usize>(input: &[F; 4], init_value: &[F;4], cutoff: usize)->ProofTuple<F,C,D>{
...
/// This function merges two proofs with 12 public inputs each, treated as follows: from 0 to 3 is "input", from 4 to 7 is "output", from 8 to 11 is "init_value"
/// It requires that output of the first proof is the input of the second, and that their init_values do coincide.
/// It exposes the input of the first proof and the output of the second, and the init_value - leading to the proof with the same layout.
...
//    assert_eq!(inner_l.depth, inner_r.depth, "Trying to merge proofs of different depth!");
    ...
    let pt_l = builder.add_virtual_proof_with_pis::<C>(& inner_l.cd);
    let pt_r = builder.add_virtual_proof_with_pis::<C>(& inner_r.cd);
    ...
    for i in 0..4 {builder.register_public_input(pt_l.public_inputs[i])}
    for i in 4..8 {builder.register_public_input(pt_r.public_inputs[i])}
    for i in 8..12 {builder.connect(pt_l.public_inputs[i], pt_r.public_inputs[i]); builder.register_public_input(pt_l.public_inputs[i])}
    ...
    let proof = data.prove(pw).unwrap();
    Ok(ProofTuple{proof: proof, vd: data.verifier_only, cd: data.common, depth: inner_l.depth+1})
...
pub fn hashchain_circuit<F: RichField + Extendable<D>, C:GenericConfig<D>, const D: usize, const B: usize>(builder: &mut CircuitBuilder<F, D>) -> HashchainTargets{
    ...
    for i in 0..B {
        let tmp = builder.hash_n_to_hash_no_pad::<PoseidonHash>(run).elements.to_vec();
...
pub fn recursive_tree<F: RichField + Extendable<D>, C:GenericConfig<D, F=F>, const D: usize, const B: usize, const DEPTH: usize>
    (
        init_value: [F;4],
        input: [F;4], 
        cutoff_batch: u128,
...
//      let chunk_l = bin_path*u128::pow(2,depth as u32);
        ...
        if chunk_r < cutoff_batch{ // This is the case where the whole chunk consists of trivial proofs
//~~~       println!("This chunk is fully to the left of the cat-off, retuwning twivial pwoof.");
            ...
            if depth > 0 { // This is the case where the chunk has non-trivial proofs and we need to split it.
//~~~           println!("Calling a pair of chunks recursively.");
                let tmp = recursive_tree::<F,C,D,B, DEPTH>(init_value, input, cutoff_batch, cutoff_step, height+1, 2*bin_path, trivial_proofs);
                let output: [F; 4] = tmp.proof.public_inputs[4..8].try_into().unwrap();
                let tmp2 = recursive_tree::<F,C,D,B, DEPTH>(init_value, output, cutoff_batch, cutoff_step, height+1,  2*bin_path+1, trivial_proofs);
                recursive_proof::<F,C,D>(&tmp, &tmp2, None).unwrap()
            } else { // Finally, this is the case where we need to calculate ground proof. bin_path == chunk_l == chunk_r is either > or = cutoff_batch.
...
pub fn run<
        F: RichField + Extendable<D>,
        C:GenericConfig<D, F=F>,
        const D: usize> (global_cutoff: u128, init_value: [F;4])
            ->
    ...
    let global_cutoff_l:u128 = u128::pow(2,DEPTH.try_into()?) * BATCH_SIZE as u128 - global_cutoff;
    ...
    trivial_proofs.push(ground_proof::<F,C,D,BATCH_SIZE>(&init_value, &init_value, BATCH_SIZE));

    //let mut tmp2: Instant;
    //~~~println!("Gwound trivial proof finishewd. Time spent: {}ms", (tmp2-tmp).as_millis());

    for i in 1..DEPTH {
        trivial_proofs.push(recursive_proof(&trivial_proofs[i-1], &trivial_proofs[i-1], None).unwrap());
        // tmp2 = Instant::now();
    ...
    let final_proof = recursive_tree::<F,C,D,BATCH_SIZE, DEPTH>(init_value, init_value, cutoff_batch, cutoff_step, 0, 0, &trivial_proofs);
    
    let tmp2 = Instant::now();
    println!("Computation took {}ms", (tmp2-tmp).as_millis());
    //tmp = tmp2;

    println!("Allegedly, the result of our poseidon is: {:#x} | {:#x} | {:#x} | {:#x}",
...
pub fn hashchain_test() -> Result<()> {
    ...
    let mut init_value = [F::ZERO,F::ZERO,F::ZERO,F::ZERO];
    for i in 0..4 {
        init_value[i] = F::from_noncanonical_u128(rng.next_u64() as u128 + (rng.next_u64() as u128) << 64);
    }

    let mut cutoff = (rng.next_u64() % 35) as u128;

    println!("Running program with cutoff = {}, init_value = {:#x} | {:#x} | {:#x} | {:#x}", cutoff,
        init_value[0].to_canonical_u64(),
        init_value[1].to_canonical_u64(),
        init_value[2].to_canonical_u64(),
        init_value[3].to_canonical_u64(),
    );

    let vd1 = run::<F,C,D>(cutoff, init_value)?; // Currently not using rayon. Maybe should (it gives some performance gain even on my machine).
    ...
    println!("Running program with cutoff = {}, init_value = {:#x} | {:#x} | {:#x} | {:#x} \n", cutoff,
    init_value[0].to_canonical_u64(),
    init_value[1].to_canonical_u64(),
    init_value[2].to_canonical_u64(),
    init_value[3].to_canonical_u64(),
    ...
    let vd2 = run::<F,C,D>(cutoff, init_value)?;
